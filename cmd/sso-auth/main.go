...
func main() {
	...
	timeoutHandler := http.TimeoutHandler(authMux, config.ServerConfig.Timeout, "")

	s := &http.Server{
		Addr:         fmt.Sprintf(":%d", config.ServerConfig.Port),
		ReadTimeout:  config.ServerConfig.TCPConfig.ReadTimeout,
		WriteTimeout: config.ServerConfig.TCPConfig.WriteTimeout,
		Handler:      auth.NewLoggingHandler(os.Stdout, timeoutHandler, config.MetricsConfig.RequestLogging, statsdClient),
	}
