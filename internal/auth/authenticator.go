...
func NewAuthenticator(config Configuration, optionFuncs ...func(*Authenticator) error) (*Authenticator, error) {
	...
	proxyRootDomains := []string{}
	for _, domain := range config.AuthorizeConfig.ProxyRootDomains {
	...
	p := &Authenticator{
		ProxyClientID:     config.ClientConfig.ClientID,
		ProxyClientSecret: config.ClientConfig.ClientSecret,
		EmailDomains:      config.AuthorizeConfig.EmailDomains,
